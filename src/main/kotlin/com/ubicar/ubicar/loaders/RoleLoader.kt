...
class RoleLoader(private val userRoleRepository: UserRoleRepository) : CommandLineRunner, Ordered {
  ...
  override fun run(vararg args: String?) {
    ...
    roles.add(
      UserRole(
        "Comprador/Vendedor", "ROLE_comprador_vendedor", "comprador_vendedor", true, LocalDate.now(), mutableListOf()
      )
    )
    ...
    roles.map {
      userRoleRepository.findFirstBySlug(it.slug)
        .orElseGet { userRoleRepository.save(it) }
    }
  }
